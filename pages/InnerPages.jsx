...
const AboutPage = ({ t, lang }) => (
  <div>
    <PageHeader
      eyebrow={lang==='id'?'Tentang kami':'About us'}
      title={lang==='id' ? 'Sebuah ruang publik dengan sikap.' : 'A public space with a stance.'}
      sub={lang==='id'
        ? 'Resonansi lahir dari keyakinan sederhana: melawan korupsi adalah pekerjaan warga — bukan hanya lembaga. Dan pekerjaan warga butuh tempat.'
        : 'Resonansi was born from a simple belief: fighting corruption is citizens\' work — not only institutions\'. And citizens\' work needs a place.'}
      tone="yellow"/>
    ...
    <section className="app" style={{ padding:'var(--gap-5) var(--gap-4)' }}>
      <div className="r-split-tall" style={{ gap: 48 }}>
        <div>
          <div className="eyebrow">{lang==='id'?'Ceritanya':'The story'}</div>
          <div style={{ marginTop: 14, fontFamily:'var(--font-serif)', fontStyle:'italic', fontSize: 34, lineHeight: 1.2, color:'var(--navy)' }}>
            {lang==='id' ? 'Dibuka pada 2022. Dibangun oleh ratusan tangan.' : 'Opened in 2022. Built by hundreds of hands.'}
          </div>
        </div>
        <div style={{ fontSize: 17, lineHeight: 1.6, color:'var(--ink-2)' }}>
          <p style={{ marginTop: 0 }}>
            {lang==='id'
              ? 'Dua dekade lebih Indonesia Corruption Watch bekerja lewat investigasi, kajian, dan advokasi. Kami menyadari satu hal: gerakan antikorupsi butuh ruang fisik. Tempat bertemu, berbagi dokumen, berlatih menulis rilis, menangis ketika kalah, lalu kembali bekerja keesokan hari.'
              : 'For over two decades ICW has worked through investigation, research, and advocacy. We realised one thing: the anti-corruption movement needs a physical place. Somewhere to meet, share documents, rehearse press releases, mourn defeats, and return to work the next day.'}
          </p>
          <p>
            {lang==='id'
              ? 'Resonansi adalah jawaban kami. Bangunan dua lantai di Kalibata yang dirombak pelan-pelan selama setahun, dengan sumbangan buku, kursi, kopi, dan tenaga. Hari ini ia menjadi rumah bagi tujuh ruangan dan ribuan kemungkinan.'
              : 'Resonansi is our answer. A two-storey building in Kalibata renovated slowly over a year, through donated books, chairs, coffee, and labour. Today it hosts seven rooms and thousands of possibilities.'}
          </p>
        </div>
      </div>
      ...
      <div className="r-grid-4" style={{ marginTop: 64, gap: 14, position:'relative' }}>
        ...
        {[
          { y:'2022', t:{id:'Pintu dibuka', en:'Doors open'} },
          { y:'2023', t:{id:'100 anggota Resocircle', en:'100 Resocircle members'} },
          { y:'2024', t:{id:'Perpustakaan diresmikan', en:'Library opens'} },
          { y:'2026', t:{id:'Fellowship angkatan ke-3', en:'3rd Fellowship cohort'} },
        ].map((m,i)=>(
...
const ServicesPage = ({ t, lang, setPage }) => (
  <div>
    <PageHeader
      eyebrow={lang==='id'?'Layanan':'Services'}
      title={lang==='id'?'Pesan ruang, jadilah anggota, duduklah.':'Book a room, become a member, sit down.'}
      sub={lang==='id'?'Tujuh ruangan, tiga jenis keanggotaan, satu filosofi: biayanya adil, semangatnya besar.':'Seven rooms, three membership tiers, one philosophy: fair cost, large spirit.'}
      tone="sky"/>

    {/* Rooms grid */}
    <section className="app" style={{ padding:'var(--gap-5) var(--gap-4)' }}>
      <SectionHeader eyebrow={lang==='id'?'01 Fasilitas ruang':'01 Room facilities'} title={lang==='id'?'Pilih ruang yang cocok':'Pick the right room'}/>
      <div className="r-grid-3" style={{ marginTop: 32, gap: 18 }}>
        {ROOMS.map(r => (
          ...
          >
            <div className="placeholder" data-tone={r.tone} style={{ height: 180 }}>
              <span className="ph-label">{r.id}.jpg</span>
            </div>
            <div style={{ padding: 20 }}>
              <span className="tag" data-tone={r.tone}><span className="dot"/>{r.type[lang]}</span>
              <div style={{ marginTop: 10, fontFamily:'var(--font-display)', fontWeight: 700, fontSize: 22, color:'var(--navy)' }}>{r.name}</div>
              <div style={{ marginTop: 6, fontSize: 13, color:'var(--muted)' }}>
                {r.cap} · {lang==='id'?'AC, proyektor, mic':'AC, projector, mic'}
              </div>
              <div style={{ marginTop: 16, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                <div>
                  <div style={{ fontFamily:'var(--font-display)', fontWeight: 700, fontSize: 18, color:'var(--navy)' }}>
                    Rp 0 <span className="mono" style={{ fontSize: 11, color:'var(--muted)', fontWeight: 400 }}>/ {lang==='id'?'jam (anggota)':'hr (member)'}</span>
                  </div>
                </div>
                <button className="btn btn-ghost" style={{ padding:'10px 14px', fontSize: 13 }}>{lang==='id'?'Cek jadwal':'Check'} <Icon name="arrow-ne" size={12}/></button>
              </div>
            </div>
          </div>
        ))}
    ...
    <section style={{ background:'var(--bg-2)', padding:'var(--gap-5) 0', borderTop:'1px solid var(--line)' }}>
      <div className="app">
        <SectionHeader eyebrow={lang==='id'?'02 Keanggotaan':'02 Membership'} title={lang==='id'?'Tiga pintu masuk':'Three ways in'}/>
        <div className="r-grid-3" style={{ marginTop: 32, gap: 18 }}>
          {[
            ...
            <div key={i} style={{
              ...
              }}>{lang==='id'?'PALING POPULER':'MOST POPULAR'}</div>}
              <div className="eyebrow" style={{ color: m.feat ? 'var(--yellow)' : 'var(--navy)', opacity: m.feat ? 1 : 0.7 }}>{m.n[lang]}</div>
              <div style={{ marginTop: 14, display:'flex', alignItems:'baseline', gap: 6 }}>
                <span style={{ fontFamily:'var(--font-display)', fontWeight: 700, fontSize: 44, letterSpacing:'-0.03em' }}>{m.p}</span>
                <span className="mono" style={{ fontSize: 12, opacity: 0.7 }}>{m.per[lang]}</span>
              </div>
              ...
              <button className={m.feat ? 'btn btn-accent' : 'btn btn-primary'} style={{ width:'100%', marginTop: 28, justifyContent:'center' }} onClick={()=>setPage('join')}>
                {lang==='id'?'Daftar sekarang':'Join now'} <Icon name="arrow"/>
              </button>
            </div>
          ))}
        </div>
...
const ActivitiesPage = ({ t, lang }) => {
  ...
  const types = [
    { k:'all',      l:{id:'Semua', en:'All'} },
    { k:'Diskusi Publik', l:{id:'Diskusi', en:'Discussion'} },
    { k:'Kelas Seni',     l:{id:'Seni', en:'Arts'} },
    { k:'Klub Baca',      l:{id:'Baca', en:'Reading'} },
    { k:'Aksi Solidaritas', l:{id:'Aksi', en:'Action'} },
  ];
        ...
        <div className="r-split-map" style={{ gap: 48 }}>
          <form onSubmit={e=>{ e.preventDefault(); setSent(true); }} style={{
            ...
            {sent ? (
              ...
              <FormField label="Surel" placeholder="kamu@surel.id" type="email"/>
